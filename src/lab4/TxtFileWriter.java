...
/**
 * TxtFileWriter is a low-level class that is used to write input data to a text
 * (.txt) file. This program uses the FileWriterStrategy interface to provide
 * the method for writing to a file. Currently, the program is rigid, as it does
 * not allow for file path to be entered as input, but requires that a specific
 * path be used for locating the file being written to. It uses the inputData
 * provided as a String and writes it into the file, exactly as it is provided
 * (no formatting).
 *
...
public class TxtFileWriter implements FileWriterStrategy {
     ...
     * .txt. It calls the getAppendStatus() method to determine if the file
    ...
    public String getFileName() {
        String txt = ".txt";  // local variable for file extension
        try {
            System.out.println("Enter the file name to write to as filename.txt");
            String fn = input.nextLine();
            if (fn != null && fn.length() != 0 && (fn.endsWith(txt))) {
                fileName = fn;
            ...
            System.out.println(FILE_EXT + txt);
            getFileName();
        }
        return null;
    }
    ...
    public static void main(String[] args) throws NullPointerException,
            DataFormatException {
        String fn = null;
        TxtFileWriter writer = new TxtFileWriter(fn);
        writer.getFileName();
